...
## 十二、Phase M 下一步（v0.3.x → v0.4）

> 更新：v0.3.0 已发布（GitHub Release + crates.io 14 crates）。栈/寄存器 VM 均 **102/102**。

### 12.1 性能实测（release，bench_fib30）

| 后端 | fib 时间 | 相对 |
|------|----------|------|
| 栈式 VM（默认） | ~115–130 ms | 1.0× |
| 寄存器 VM | ~2000–2700 ms | **~15–20× 慢** |

结论：功能已对齐，但寄存器 VM **尚不能作为默认后端**。默认保持栈式。

### 12.2 Phase M 优先级

| ID | 项 | 优先级 | 说明 |
|----|----|--------|------|
| **M1** | 寄存器 VM 性能 | **P0** | 目标：fib ≤ 2× 栈式；热路径 Call/Ret/LoadLocal/算术 |
| **M2** | 文档站部署全绿 | **P1** | VitePress config 已修，确认 Pages 发布成功 |
| **M3** | 泛型类单态化或明确文档 | **P2** | 当前字段擦除 |
| **M4** | async 决策 | **P3** | 无 IoT 场景继续冻结 |
| **M5** | 考虑切换默认后端 | **P3** | 仅当 M1 达标后 |

### 12.3 验收（v0.4）

- [ ] 寄存器 VM fib ≤ 2× 栈式，且 102/102
- [ ] 文档站 CI 全绿
- [ ] 性能报告含双后端对比
