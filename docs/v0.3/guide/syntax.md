...
模块即文件。用 `import "…"` 导入，导出项通过模块名限定访问：

### 导入

```
import "math"          // 按模块名（registry / mailang.toml 依赖）
import "./utils"       // 相对路径导入同级文件

// 使用（以文件名/模块名为命名空间）
let hypotenuse = math.sqrt(3.0 ** 2 + 4.0 ** 2)
println(utils.greet("MaìLang"))
```

### 定义模块

将函数/常量写在独立 `.mai` 文件中即可，无需额外声明：
...
> 项目级依赖与包管理见 [包注册表](/v0.3/guide/registry) 与 `mailang.toml`（`mailang deps`）。
> `module name { … }` 内联模块块与 `pub fn` 访问修饰符**尚未支持**。
