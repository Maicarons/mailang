...
> **项目链接**：[GitHub](https://github.com/Maicarons/mailang) · [Playground](https://maicarons.github.io/mailang/playground)
...
| `mailang build file.mai` | 编译到 `.mailangbc` |
| `mailang fmt file.mai` | 格式化源文件（`--check` 仅检查） |
| `mailang deps` | 解析 `mailang.toml` 路径依赖 |
| `mailang publish` / `install` / `search` / `yank` / `registry` | 文件系统 / 静态 HTTP 包注册表 |
