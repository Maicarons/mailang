...
MaìLang 默认使用**栈式虚拟机 + slot 局部变量**（可选寄存器后端 `--vm=register`，特性与栈式 VM 对齐）。操作数走栈，局部变量通过帧内 slot 索引访问：
...
- [解析器详解](/v0.3/reference/parser) - 语法分析细节
- [类型系统](/v0.3/reference/types) - 类型定义
- [字节码参考](/v0.3/reference/bytecode) - 完整操作码列表
