...
> **项目链接**：[GitHub](https://github.com/Maicarons/mailang) · [解析器源码](https://github.com/Maicarons/mailang/tree/master/crates/mailang-parser)
...
解析器支持 token 同步点恢复：`parse_program_recovering` 会收集**多条**语法错误并返回部分 AST。
...
同步点：括号深度为 0 时的换行、语句起始关键字（`let`/`fn`/`class` 等）、或 `}` 闭合嵌套块后。`eval`/`run` 仍使用 fail-fast 的 `parse_program`；LSP 使用恢复模式一次报多错。
```
...
- [编译原理](/v0.3/reference/compiler) - 整体编译流程
- [类型系统](/v0.3/reference/types) - 类型定义
