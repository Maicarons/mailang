...
> **相关**：[IoT 部署](/guide/iot) · [真实硬件部署](/guide/hardware) · 示例脚本 [`examples/iot_blink.mai`](https://github.com/Maicarons/mailang/blob/master/examples/iot_blink.mai)
...
> `mailang-ffi` 依赖 `std`，适合 **ESP-IDF（有 OS/heap）**。裸机 Cortex-M4 请改用 `mailang-vm` + `no_std + alloc`（见 [IoT 部署](/guide/iot)）。
...
C 固件常见做法：嵌入源码用 `mailang_eval`，或用 Rust 入口跑 `.mailangbc`、C 只做板级初始化（详见 [真实硬件部署](/guide/hardware)）。
...
- [真实硬件部署](/guide/hardware) · [IoT 部署](/guide/iot)
