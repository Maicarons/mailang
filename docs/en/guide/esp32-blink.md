...
> **See also**: [IoT Deployment](/en/guide/iot) · [Real Hardware](/en/guide/hardware) · script [`examples/iot_blink.mai`](https://github.com/Maicarons/mailang/blob/master/examples/iot_blink.mai)
...
> `mailang-ffi` needs `std` (ESP-IDF with OS/heap). For bare-metal Cortex-M4 use `mailang-vm` + `no_std + alloc` instead (see [IoT Deployment](/en/guide/iot)).
...
Typical C firmware options: embed source with `mailang_eval`, or run `.mailangbc` from a Rust entry while C only does board init (see [Real Hardware](/en/guide/hardware)).
...
- [Real Hardware](/en/guide/hardware) · [IoT Deployment](/en/guide/iot)
